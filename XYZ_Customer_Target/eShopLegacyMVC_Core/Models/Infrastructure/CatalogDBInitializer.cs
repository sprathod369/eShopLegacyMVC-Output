...
namespace eShopLegacyMVC_Core.Models.Infrastructure
{
    public class CatalogDBInitializer : IDatabaseInitializer<CatalogDBContext>
    {
        ...
        public CatalogDBInitializer(CatalogItemHiLoGenerator indexGenerator, ILogger<CatalogDBInitializer> logger, bool useCustomizationData)
        {
        ...
        {
            var rawQuery = context.Database.ExecuteSqlRaw($"SELECT NEXT VALUE FOR {dBSequenceName}");
            var sequenceId = Convert.ToInt32(rawQuery);
            return sequenceId;
        }
